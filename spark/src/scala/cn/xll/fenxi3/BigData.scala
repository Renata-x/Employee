...
object BigData {
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession = SparkSession
      .builder()
      .appName("Spark SQL JDBC bigdata")
      .config(sparkConf)
      .enableHiveSupport()
      .getOrCreate()
    ...
    val df = sparkSession.read
      .schema(schema)
      .option("header", "true")
      .csv("hdfs://spark01:8020/EmployeeBd/demo.csv")

    // 选择 job_name 等于 'Web' 的数据
    val filteredDf = df.filter(df("job_name") === "大数据")


    filteredDf.write.mode(SaveMode.Overwrite).saveAsTable("employee.bigdata")
